...
<body>
	...
	<table width="50%">
		...
		<tr>
			<td colspan="5" class="reportTitle">Revenue and Expense Per Truck <br/> <c:out value="${expense.date_from}"/> &nbsp;&nbsp;  - &nbsp;&nbsp;  <c:out value="${expense.date_to}"/></td>
		</tr>
		<tr>
		...
		<c:forEach var="expRevPerTruckReport" items="${expRevPerTruckReport}">
			<tr <c:choose><c:when test="${rowNum%2 == 0}">style="line-height: 35px; background-color: #e5e5e5;"</c:when><c:otherwise>style="line-height: 35px;"</c:otherwise> </c:choose>>
				...
				<td>${expRevPerTruckReport.frightOrder.truckInformation.truck_plate_no}</td>
				<td align="right">
					<c:set var="total_revenue" value="${expRevPerTruckReport.total_revenue}"/>
					<fmt:formatNumber minFractionDigits="2" pattern="00,000.00" value="${total_revenue}"/>
				</td>
				<td align="right">					
					<c:set var="total_expense" value="${expRevPerTruckReport.total_expense}"/>
					<fmt:formatNumber minFractionDigits="2" pattern="00,000.00" value="${total_expense}"/>
				</td>
				<td align="right">					
					<c:set var="net_diff" value="${expRevPerTruckReport.total_revenue - expRevPerTruckReport.total_expense}"/>
