...
public class Excel_RevenueExpensePerTruck {
	...
	private void createLabel(WritableSheet sheet) throws WriteException{
		...
		addTitle(sheet, 0, 1, "Revenue and Expense Per Truck", Alignment.CENTRE);
	...
	private void createContent(WritableSheet sheet) throws WriteException, RowsExceededException{
		...
		for(int i = 0, j = 4; i < expRevPerTruckReport.size(); i++, j++){ //row
			...
			addLabel(sheet, 1, j, expRevPerTruckReport.get(i).getFrightOrder().getTruckInformation().getTruck_plate_no());
	...
	public static byte[] excelRequiredInfo(String filePath, List<Expense> expRevPerTruckReport){
		...
		Excel_RevenueExpensePerTruck test = new Excel_RevenueExpensePerTruck();
