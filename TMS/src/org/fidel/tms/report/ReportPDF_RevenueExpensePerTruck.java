...
public class ReportPDF_RevenueExpensePerTruck {
	...
	public static byte[] generatePDFDocument(String FileName, List<Expense> expRevPerTruckReport, Expense expense){
		...
		public static PdfPTable PDFBodyContent(){
			...
			for(int i = 0; i < expRevPerTruckReport.size(); i++){
				
				rev_grand_total += expRevPerTruckReport.get(i).getTotal_revenue();
				exp_grand_total += expRevPerTruckReport.get(i).getTotal_expense();
				
				double netDiff = expRevPerTruckReport.get(i).getTotal_revenue() - expRevPerTruckReport.get(i).getTotal_expense();
				grand_total += netDiff;
				
				int rowNum = i + 1;
				insertCell(pdfContent, String.valueOf(rowNum), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, expRevPerTruckReport.get(i).getFrightOrder().getTruckInformation().getTruck_plate_no(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, String.format("%,.2f", expRevPerTruckReport.get(i).getTotal_revenue()), Element.ALIGN_RIGHT, 1, 1, tblContentFont);
				insertCell(pdfContent, String.format("%,.2f", expRevPerTruckReport.get(i).getTotal_expense()), Element.ALIGN_RIGHT, 1, 1, tblContentFont);
		...
		public static PdfPTable PDFHeaderContent(){
			...
			cell = new PdfPCell(new Phrase("Revenue and Expense Per Trucks", headerFont));
			cell.setColspan(1);
			cell.setBorder(0);
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pdfHeader.addCell(cell);
...
//		}
		...
		public static void setExpRevPerTruckReport(List<Expense> expRevPerTruckReport) {
			ReportPDF_RevenueExpensePerTruck.expRevPerTruckReport = expRevPerTruckReport;
		}
		...
		public static void setExpense(Expense expense) {
			ReportPDF_RevenueExpensePerTruck.expense = expense;
		}
}
