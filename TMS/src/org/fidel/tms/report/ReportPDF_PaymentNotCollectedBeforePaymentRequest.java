...
public class ReportPDF_PaymentNotCollectedBeforePaymentRequest {
	...
	private static List<FrightOrder> unprocessedFrightOrder;
		
	public static byte[] generatePDFDocument(String FileName, List<FrightOrder> unprocessedFrightOrder){
			
			setUnprocessedFrightOrder(unprocessedFrightOrder);
		...
		public static PdfPTable PDFBodyContent(){
			...
			insertCell(pdfContent, "Fright Order Close Date", Element.ALIGN_CENTER, 1, 1, tblHeaderFont);
			...
			for(int i = 0; i < unprocessedFrightOrder.size(); i++){
				...
				Date date_from = unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getClose_date();
				...
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getClient_organization(), Element.ALIGN_LEFT, 1, 1, tblContentFont);
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getAssociations().getAssociation_name(), Element.ALIGN_LEFT, 1, 1, tblContentFont);
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getFon(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getTruckInformation().getTruck_plate_no(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				
				insertCell(pdfContent, String.valueOf(unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getPrice()), Element.ALIGN_RIGHT, 1, 1, tblContentFont);
				insertCell(pdfContent, String.valueOf(unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getDelivered_quantity()), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, String.valueOf(unprocessedFrightOrder.get(i).getCommission()), Element.ALIGN_RIGHT, 1, 1, tblContentFont);
				
				double price = unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getPrice();
				double del_qty = unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getDelivered_quantity();
				double comm = unprocessedFrightOrder.get(i).getCommission();				
				...
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getDate_from(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getDate_to(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
				insertCell(pdfContent, unprocessedFrightOrder.get(i).getFrightOrderTripDetail().getClose_date().toString(), Element.ALIGN_CENTER, 1, 1, tblContentFont);
		...
		public static PdfPTable PDFHeaderContent(){
			...
			cell = new PdfPCell(new Phrase("Fright Order Not Requested for Payment", headerFont));
			cell.setColspan(1);
			cell.setBorder(0);
			cell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pdfHeader.addCell(cell);
...
//		}
		...
		public static List<FrightOrder> getUnprocessedFrightOrder() {
			return unprocessedFrightOrder;
		}

		public static void setUnprocessedFrightOrder(List<FrightOrder> unprocessedFrightOrder) {
			ReportPDF_PaymentNotCollectedBeforePaymentRequest.unprocessedFrightOrder = unprocessedFrightOrder;
		}
}
